...
onMounted(() => {
  ...
  .then(response => {
    ...
    doc.value = response
    focusedEntities.value = get.value(doc.value._source, appSettings.esEntitiesField)
    loaded.value = true
  })
  .catch(err => {
    console.warn(err);
...
<template>
  <main style="display: flex; flex-grow: 1; overflow: hidden">
    <div style="flex-grow: 1; display: flex; flex-direction: column">
      <div class="headline"><h1>ADD | DOC <span v-if="loaded">| {{ get(doc._source, appSettings.esTitleField) }}</span></h1></div>
      ...
      <div style="background-color: #000; flex-grow: 1; display: flex" v-if="loaded">
        <basemapPanel
